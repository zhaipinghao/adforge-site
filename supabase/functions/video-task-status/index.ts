...
Deno.serve(async (req: Request) => {
  ...
  if (req.method === "POST") {
    ...
    return jsonResponse(req, { ok: false, error: "只接受 GET/POST 請求。" }, 405);
  }

  const taskId = urlTaskId || String((body as Record<string, unknown>).taskId || (body as Record<string, unknown>).requestId || "").trim();
  if (!taskId) {
    return jsonResponse(req, { ok: false, error: "缺少 taskId。" }, 400);
  }
  ...
  for (const base of candidates) {
    const endpoint = `${base}/api/video/status/${encodeURIComponent(taskId)}`;
