(() => {
  ...
  const API_STATUS = {
    ...
    note: "[NEEDS_REPLACEMENT] Source page had no real fetch/XHR/WebSocket calls. This static page uses mock generation until ADFORGE backend/API is connected."
  };

  window.ADFORGE_IMAGE_TO_VIDEO_AUDIT = {
    source: "https://8080-ir4behi45lcej1g3e11n5-583b4d74.sandbox.novita.ai/image-to-video.html",
    apiCallsFound: [],
  ...
  const faqs = [
    {
      q: "現在這個首頁會真的生成影片嗎？",
      a: "目前是靜態 mock 流程，用來展示未來產品體驗。真實生成會等後端 API、付款與人工審核流程穩定後再開放。"
    },
    {
      q: "為什麼不直接把 API 放在前端？",
  ...
  function initGenerator() {
    ...
    button.addEventListener("click", () => {
      // TODO [NEEDS_REPLACEMENT]: connect this button to the real ADFORGE image-to-video backend.
      // Original sandbox page did not expose a real API endpoint; it used local UI logic only.
      if (typeof gtag === "function") {
        gtag("event", "start_mock_video_generation", { event_category: "image_to_video" });
      }
